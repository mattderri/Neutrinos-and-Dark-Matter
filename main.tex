...
Another evidence comes from \textbf{rotational velocity} of galaxies. The surface brightness is given by $I(R)=I_0e^{-R/R_s}$. It typically falls off exponentially with increasing distance from the center. For $R>R_s$ we are basically enclosing all the visible mass of the galaxy. For our galaxy, $R_s=4$\,kpc. If we are at a distance $R>R_s$, from the Gauss law we get:
\[
\frac{v^2}{R}=\frac{GM(R)}{R^2}\to v=\sqrt{\frac{GM(R)}{R}}
\] 
However, this is not what is measured: this is telling us that the mass we are enclosing cannot be just the visible one.\\
